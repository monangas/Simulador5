...
if (isset($_SESSION['usuario'])) {
    ...
    <script type="text/javascript">
        function eliminarBebida(i) {
            ...
            xhttp.open("GET", "bebidas/eliminarBebidas.php?i="+i, true);
            xhttp.send();
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<tr><td>" . $row["idbebidas"] . "</td><td>" . $row["nombre"] . "</td><td>" . $row["presentacion"] . "</td><td>";
        if (isset($_SESSION['usuario'])) {
            echo "<button type='button' class='btn btn-danger'><span class='glyphicon glyphicon-remove' onclick='eliminarBebida(" . $row["idbebidas"] . ")'></span></button> ";
        }
        echo "</td></tr>";
    }
} else {
